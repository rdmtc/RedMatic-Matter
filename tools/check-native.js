...
/* Fails when the installed production dependency tree contains anything
   ...
   prebuilt .node addons or optionalDependencies (ROADMAP D-2). Copied from
   RedMatic-HomeKit with one allowance: `@matter/nodejs` is an
   optionalDependency of `@matter/main` by design (the Node.js platform
   layer) and is pure JavaScript, so it is allowed as long as it stays that
   way — its files are scanned like every other package.
...
const ALLOWED_OPTIONAL = new Set(['@matter/nodejs']);
...
for (const [pkgPath, entry] of Object.entries(lock.packages)) {
    ...
    if (entry.optional && !ALLOWED_OPTIONAL.has(name)) {
...
if (problems.length > 0) {
    console.error('native/binary dependencies found (ROADMAP D-2):');
