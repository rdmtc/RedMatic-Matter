...
Status 2026-09-02: **research and planning only, nothing implemented.** The
repository is empty apart from this file. Research basis: `../hm2matter`
(the abandoned standalone CCU addon, whose matter.js work is ported here),
`../RedMatic-HomeKit` (4.0.0-dev.7, the UX and structure template),
`../node-red-contrib-ccu` (4.1.0-dev.5, the data source),
`../RedMatic` (9.0.0-dev.12: Node 24.20 / Node-RED 5.0.6, no native modules,
only node-red-contrib-ccu bundled), matter.js 0.17.9 as installed in
hm2matter, and the web research summarised in appendix A.
...
door: Matter 1.5 *Closure* is the right device type but no controller
...
- **Accessory identity = CCU address.** HomeKit: `uuid(address)`. Matter:
  ...
  address, `uniqueId` derived; matter.js persists the endpoint *number*
...
| OQ-2  | **closed 2026-09-02 by D-16**: no garage node. Matter 1.5 *Closure* would be the right device type, but no controller renders it (matterbridge-homematic HM-9 research); a cover/lock stand-in was rejected by the maintainer.                                                            | Garage doors stay on RedMatic-HomeKit. HmIP-MOD-HO/WGC channels are listed as "no Matter equivalent" in the device list. Task 12 holds the trigger for revisiting.                                                                                                                                                              |
...
- `matter-devices.js`: hm2matter's definitions plus what task 9's universal
  ...
  air quality, water valve, on/off light switch is *not* offered — it is a
...
- **Garage**: Matter ≤ 1.4 has no garage door device type; 1.5 introduced
  *Closure* (0x0230), which covers garage doors and gates and carries
...
- **RedMatic** (confirmed 2026-09-02, OQ-7): bring up the CCU3's IPv6
...
- Device verification with the maintainer's CCU (64 device types) via
  node-red-contrib-ccu; reporter feedback for HmIP-DLD, BROLL-2, eTRV-*,
  WTH-*, SMI55, DRDI3.
...
- mDNS: `UdpMulticastServer` binds udp4 + udp6 on 5353 with
  ...
  *without* reuse since PR #2547 (2025-10), so two bridges need two ports.
