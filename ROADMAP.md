...
- **Round 3**: the Home app showed the 7 accessories again after the 50
  virtual buttons had come and gone. **Coexistence re-browse (OQ-5)
  positive** — both bridges answer from one process. **Type change on a
  redeploy rotates the identity as designed (D-17)**: switch node "plug
  → light" replaced endpoint 4 (`mt_sw~onOffPlugInUnit`) by 59
  (`mt_sw~onOffLight`) without a restart, she showed device type 256 at
  the new endpoint at once; "light → plug" then gave endpoint 60, not 4,
  because the replaced identity's storage is erased — a controller would
  treat it as a new device either way (document in the help: type changes
  mean re-adding the accessory in the controller). Note for scripted
  deploys: a `POST /flows` with a stale `rev` is rejected silently by the
  Node-RED API (409) — always re-fetch before deploying.
